...
def train_model(model, train_data, valid_data, fields, optim):
    ...
    def train_epoch(epoch):
        ...
        for i, batch in enumerate(train):
            ...
            plot_every = 50
            if i % plot_every == 0:
                plot_array["train"]["ppl"].append(total_stats.ppl())
                plot_array["train"]["acc"].append(total_stats.accuracy())
                
                valid_stats = eval(model, criterion, valid_data, fields)
                plot_array["val"]["ppl"].append(valid_stats.ppl())
                plot_array["val"]["acc"].append(valid_stats.accuracy())
                
                outfile = open('%s_acc_%.2f_ppl_%.2f_e%s_batch_%s.pt' % (plot_folder_path, valid_stats.accuracy(), valid_stats.ppl(), epoch, i), "w")
                outfile.write(json.dumps(plot_array))
                outfile.close()
            '''             
    ...
    '''
    # For tuning parameters
    plot_array = {"train":{"ppl":[], "acc":[]}, "val":{"ppl":[], "acc":[]}}
    plot_folder_path = "./data/squad/att_s2s/raining_results/"
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        # train_stats = train_epoch(epoch, plot_array, plot_folder_path)
...
def main():
    ...
    model = onmt.ModelConstructor.make_base_model(opt, model_opt, fields, checkpoint)
    print(model)
